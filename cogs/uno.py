...
class Uno(commands.Cog):  # this game is played in dms with 2-4 players
    ...
    async def uno(self, ctx: Context):
        "The root command for Uno."
        embed = Embed(title="Uno In-Game Commands📝", color=Colour.red())

        embed.add_field(
            name="In-Game Commands ⚙️",
            value=
            f"**{prefix}u c @user1 @user2 @user3** (challenge users to a game of Uno)\n"
            f"**{prefix}u d pos** (place a card down '{prefix}u d g8'\n"
            f"**{prefix}u s** (forfeit the game)\n"
            f"**{prefix}u time** (win, if the opponent hasn't played in 100 seconds)\n"
            f"**{prefix}u t** (propose a tie to your opponent)\n")

    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.command(parent=uno, aliases=['c'])
    async def challenge(self, ctx: Context, *users: User):
        "Challenges someone for a game of Uno."
    ...
    @commands.command(parent=uno, aliases=["d"])
    @commands.dm_only()
    async def drop(self, ctx: Context, card_name: str, *args: str):
    ...
    @commands.command(parent=uno, aliases=['p'])
    @commands.dm_only()
