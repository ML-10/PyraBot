...
class Battleships(commands.Cog):
    ...
    @commands.group(aliases=['bs', 'battleship', 'ship'])
    async def battleships(self, ctx: Context):
        "The root command for Battleships."

    @commands.cooldown(1, 5, commands.BucketType.user)
    @commands.command(parent=battleships, aliases=['c'])
    async def challenge(self, ctx: Context, user: User):
        "Challenges someone for a game of Battleships."
        ...
        embed.add_field(
            ...
            value=f"**{prefix}bs c @user** (challenge someone to a battleships game)\n"
                  f"**{prefix}bs sh pos** (fire your guns at pos) (Ex -> {prefix}bs sh j6\n"
                  f"**{prefix}bs r** (change your fleet, up to 3 times)\n"
                  f"**{prefix}bs time** (win, if your opponent has not played for more than 100 seconds)\n"
                  f"**{prefix}bs f** (view your fleet)\n"
                  f"**{prefix}bs s** (surrender the game)\n"
                  f"**{prefix}bs t** (propose a tie, if both of you agree the game ends in a tie)\n"
        )
    ...
    @commands.command(parent=battleships, aliases=['sh'])
    async def shoot(self, ctx: Context, pos: str):
        "Attacks at a given location."
        try:
    ...
    @commands.command(parent=battleships, aliases=['r'])
    async def reroll(self, ctx: Context):
        "Randomizes your fleet."
        try:
    ...
    @commands.command(parent=battleships, aliases=['time', 'to'])
    ...
    @commands.command(parent=battleships, aliases=['fleet', 'f'])
    async def myfleet(self, ctx: Context):
        "Shows your current fleet."
        try:
    ...
    @commands.command(parent=battleships, aliases=['resign', 'giveup', 'forfeit', 's'])
    ...
    @commands.command(parent=battleships, aliases=['t'])
    ...
    async def bchallenge_error(self, ctx: Context, error: Exception):
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await ctx.reply("**You need to specify the user!\n"
                            f"Example: {prefix}bs challenge @user**")
